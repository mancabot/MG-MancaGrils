...
module.exports = msgHndlr = async (aksa, message) => {
    try {
        ...
        const ownerNumber = ["6283815956151@c.us", "6283815956151@c.us"] // replace with your whatsapp number
            ...
            for (let i of limit) {
                if (i.id === id) {
                    ...
                    if (limits >= limitCount) {
                        found = true;
                        aksa.reply(dari, 'Perintah MANCABOT anda sudah mencapai batas, coba esok hari :)', id)
                        return true;
                    } else {
                        limit
                        found = true;
        ...
        if (body === `${prefix}mute` && isMuted(chatId) == true) {
            if (isGroupMsg) {
                if (!isGroupAdmins) return aksa.reply(dari, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Manca!', id)
        ...
        if (args.includes('@6283815956151')) { //replace with your bot number
        ...
        if (body === `${prefix}public`) {
            if (!isOwner) return aksa.reply(dari, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Manca!', id)
        ...
        if (isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwner) {
            switch (command) {
                ...
                case `${prefix}self`:
                    ...
                    if (!isOwner) return aksa.reply(dari, 'Perintah ini hanya bisa di gunakan oleh Owner Manca!', id)
                    ...
                    break
                case `prefix`:
                    aksa.reply(dari, `*Manca is Use ( ${prefix} ) Prefix!.* 
_Prefix adalah tanda di awal perintah._
_Contoh: ${prefix}menu_`, id)
                    break
                case `${prefix}setprefix`:
                    if (!isOwner) return aksa.reply(dari, 'Maaf, Fitur ini hanya untuk OWNER dan ADMIN Manca!', id)
...
Contoh: ${prefix}setprefix #`, id)
                ...
                case `${prefix}setgrouplimit`:
                    if (!isOwner) return aksa.reply(dari, 'Perintah ini hanya untuk Owner Manca!', id)
                    ...
                    break
                case `${prefix}seticon`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user MancaBot, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    ...
                    if (!isBotGroupAdmins) return aksa.reply(dari, 'Gagal, silahkan tambahkan Manca sebagai admin grup!', id)
                ...
                case `${prefix}clearall`:
                    ...
                    break
                case `${prefix}join`:
                    if (!isOwner) return aksa.reply(dari, 'Silakan hubungi Owner untuk mengundang MancaBot ke dalam Grup. Ketik !owner', id)
                    ...
                    if (check.size < minMem) return aksa.reply(dari, 'Member group tidak melebihi 2, Manca tidak bisa masuk', id)
                    if (check.status === 200) {
                        await aksa.joinGroupViaLink(link).then(() => aksa.reply(dari, 'MancaBot akan segera masuk!'))
                    } else {
...
❉──────────────────❉\n\n${bctxt}`
                    ...
                    break
                case `${prefix}getses`:
                    if (!isOwner) return aksa.reply(dari, 'Fitur ini hanya untuk Owner Manca!, id')
                    const sesPic = await aksa.getSnapshot()
                    aksa.sendFile(dari, sesPic, 'session.png', 'Nih...', id)
                    break
                case `${prefix}listblock`:
                case `${prefix}listblok`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Manca, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
...
❉──────────────────❉\n`
                    ...
                    break
                case `${prefix}ban`:
                    if (!isOwner) return aksa.reply(dari, 'Hanya Owner Manca yang bisa!:p', id)
                    for (let i = 0; i < mentionedJidList.length; i++) {
...
Total user terdaftar : ${regis.length}
          ...
          ║▌│█║▌│ █║▌│█│║▌║
                ...
                case `${prefix}unreg`: //menghapus nomor dari database
                    if (!isOwner) return aksa.reply(dari, 'Fitur ini hanya dapat digunakan oleh Owner Manca')
                    if (args.length === 1) return aksa.reply(dari, 'Masukkan nomornya, *GUNAKAN AWALAN 62* contoh: 6283815956151)
...
Ayo bergabung bersama ratusan orang lainnya ke dalam Group Official Informasi Manca!
Ketik aja !mancagroup

Follow juga ig aku yuk!
https://instagram.com/thenay_xploit_
`, id)
                            var found = true
                        }
                    }
...
*Provider :* INDIHOME ASU`, id)
                ...
                case `${prefix}findstiker`:
                    if (!isRegis) return tobz.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Manca, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
...
➸ *Cara:* ${cara}`
                ...
                case `${prefix}linkgroup`:
                    if (!isBotGroupAdmins) return aksa.reply(dari, 'Perintah ini hanya bisa digunakan ketika Manca menjadi admin', id)
                    if (isGroupMsg) {
                ...
                case `${prefix}kick`:
                    ...
                    await aksa.sendText(dari, `Berdasar perintah ${pushname}, *Manca* mengeluarkan:\n${mentionedJidList.join('\n')}`)
                    for (let i = 0; i < mentionedJidList.length; i++) {
                ...
                case `${prefix}promote`:
                    ...
                    if (!isGroupMsg) return aksa.reply(dari, 'Fitur ini hanya bisa digunakan dalam group', id)
                    if (!isGroupAdmins) return aksa.reply(dari, 'Fitur ini hanya bisa digunakan oleh admin group', id)
                    if (!isBotGroupAdmins) return aksa.reply(dari, 'Fitur ini hanya bisa digunakan ketika Manca menjadi admin', id)
                ...
                case `${prefix}odel`:
                    if (!isGroupMsg) return aksa.reply(dari, 'Fitur ini hanya bisa digunakan dalam group', id)
                    if (!isAdmin) return aksa.reply(dari, 'Fitur ini hanya bisa digunakan oleh admin Manca', id)
                    if (!quotedMsg) return aksa.reply(dari, 'Salah!!, kirim perintah *!delete <tagpesanbot>*', id)
                    if (!quotedMsgObj.fromMe) return aksa.reply(dari, 'Salah!, Bot tidak bisa mengahpus chat user lain!', id)
                    aksa.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
                    break
...
Twitter: https://twitter.com/bdrsmsdn`)
                ...
                case `${prefix}owner`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Manca, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    aksa.sendContact(dari, '6283815956151@c.us') //ganti
                    break
                case `${prefix}bot1`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Manca, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    aksa.sendContact(dari, '6283815956151@c.us') //bot gue ini
                    break
                case `${prefix}bot2`:
                    if (!isRegis) return aksa.reply(dari, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Manca, untuk pendaftaran bisa menggunakan *!regis* |nama|no hp. Contoh: !regis |${pushname}|${serial.replace(/@c.us/g,'')}`, id)
                    aksa.sendContact(dari, '62853815956151@c.us') //ini juga
                    break
                case `${prefix}bugreport`:
                    ...
                    if (isGroupMsg) {
                        ...
                        aksa.reply(dari, 'Masalah telah di laporkan ke owner Manca, laporan palsu/main2 tidak akan ditanggapi.', id)
                    } else {
                        aksa.sendText(ownerNumber, `*[BUG REPORT]*\n*WAKTU* : ${time}\n*Nama Pengirim* : ${pushname}\n\n${bug}`)
                        aksa.reply(dari, 'Masalah telah di laporkan ke owner Manca, laporan palsu/main2 tidak akan ditanggapi.', id)
                    }
