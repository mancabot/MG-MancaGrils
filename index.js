...
const start = async(aksa = new Client()) => {
    console.log('------------------------------------------------')
    console.log(color(figlet.textSync('MANCA BOT', { horizontalLayout: 'full' })))
    console.log('------------------------------------------------')
    console.log('[DEV] GrilsManca')
    console.log('[SERVER] MANCABOT IS ONLINE!')
    ...
    aksa.onAddedToGroup(async(chat) => {
        if (isWhite(chat.id)) return aksa.sendText(chat.id, `Halo aku Manca, Ketik ${prefix}help Untuk Melihat List Command Ku...`)
        if (mtcState === false) {
            const groups = await aksa.getAllGroups()
                // BOT group count less than
            if (groups.length > groupLimit) {
                await aksa.sendText(chat.id, 'Maaf, Batas group yang dapat MancaBot tampung sudah penuh').then(async() => {
                ...
                if (chat.groupMetadata.participants.length < memberLimit) {
                    ...
                    if (!chat.isReadOnly) aksa.sendText(chat.id, `Halo aku Manca, Ketik ${prefix}help Untuk Melihat List Command Ku...`)
                }
            }
        } else {
            await aksa.sendText(chat.id, 'MancaBot sedang maintenance, coba lain hari').then(async() => {
                aksa.deleteChat(chat.id)
                aksa.leaveGroup(chat.id)
            })
        }
